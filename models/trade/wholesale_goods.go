...
type WholesaleGoods struct {
	...
	DeStocks  int32   `gorm:"not null;default:0;comment:库存扣减数"`                  // 库存扣减数
	Mark      string  `gorm:"size:255;not null;default:''"`                      // 备注
	OrderID   string  `gorm:"index;column:order_id;size:40;not null;default:''"` // 批发单ID
